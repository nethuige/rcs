<?php if (!defined('THINK_PATH')) exit(); /*a:1:{s:65:"G:\EPP4\RCS\public/../application/admin\view\auth_rule\index.html";i:1489935527;}*/ ?>
<!DOCTYPE html>
<html>
<head>
    <meta charset="utf-8">
    ...
    <title>规则管理</title>
    <meta name="keywords" content="规则管理">
    ...
    <!--bootstrap-table-tree-column-->
    <link href="__PUBLIC__/css/bootstrap-table-tree-column.css" rel="stylesheet">
    <script src="__PUBLIC__/js/bootstrap-table-tree-column.js"></script>
    <!--layer-->
    ...
    <script>
        ...
        $(function () {
            ...
            $("body").on("click", ".del-btn", function () {
                ...
                var index = layer.confirm('确定要删除吗？', {
                        ...
                        var params = {id: id};
                        $.post("<?php echo url('auth_rule/delete'); ?>", params, function (data) {
                            layer.close(index);
                            if (data.status) {
            ...
            $("body").on("click", ".create-btn", function () {
                ...
                var layerIndex = layer.open({
                    ...
                    maxmin: true,
                    closeBtn: 1,
                    content: $("#auth_rule-form"),
                    btn: ['提交'],
                    yes: function (index, layero) {
                        if ($("#form1 #title").val() == "") {
                            toastr.warning("请填写权限名称", "系统提示");
                            return false;
                        }
                        ...
                        $.post("<?php echo url('auth_rule/create'); ?>", params, function (data) {
                            if (data.status) {
            ...
            $("body").on("click", ".edit-btn", function () {
                var id = $(this).data("id");
                var params = {id: id}
                $.get("<?php echo url('auth_rule/edit'); ?>", params, function (data) {
                ...
                var layerIndex = layer.open({
                    ...
                    yes: function (index, layero) {
                        ...
                        params.id = id;
                        params.status = params.status == "" ? 0 : 1;
                        $.post("<?php echo url('auth_rule/edit'); ?>", params, function (data) {
                            if (data.status) {
                                layer.close(layerIndex);
                                toastr.success(data.msg, "系统提示");
                                $table.bootstrapTable('refresh', {silent: true});
        ...
        function initTable(setParams) {
            ...
            $table.bootstrapTable({
                url: "<?php echo url('auth_rule/index'); ?>",
                method: 'post',
                ...
                pageList: [300, 500, 1000, 2000],  //可供选择的每页的行数（*）
                sortable: true, //启用排序
                sortName: "sort", //初始排序列名
                ...
                columns: [{
                    ...
                    formatter: function (value, row, index) {
                        if (row.pid == 0)return '<span style="font-weight:600;">' + value + '</span>';
                        return value;
                    }
                }, {
                    field: 'name',
                    ...
                    field: 'condition',
                    title: '<span class="tooltips" data-toggle="tooltip" title="如定义{score}>5  and {score}<100  表示用户的分数在5-100之间时这条规则才会通过">附加规则 <i class="fa fa-info-circle"></i> </span>',
                    align: 'center',
                }, {
                    field: 'create_time',
                    title: '创建时间',
                    ...
                    align: 'center',
                    formatter: function (value, row, index) {
                        return '<div class="btn-group"><a href="javascript:void(0);" class="btn btn-outline btn-xs btn-default create-btn" data-pid="' + row.id + '"><i class="fa fa-edit"></i> 子规则</a><a href="javascript:void(0);" data-id="' + row.id + '" class="btn btn-outline btn-xs btn-default edit-btn"><i class="fa fa-edit"></i> 编辑</a> <a href="javascript:void(0);" class="btn btn-outline btn-xs btn-default del-btn" data-id="' + row.id + '"><i class="fa fa-times"></i> 删除</a></div>';
                    }
                }
                ],
            ...
            <div class="ibox float-e-margins">
                ...
                <div class="ibox-content">
                    ...
                    <div class="row">
                        <div class="col-sm-12">
                            <div id="toolbar">
                                <a class="btn btn-primary create-btn" href="javascript:void(0);" data-pid="0"><i
                                        class="fa fa-plus"></i> 创建规则</a>
                                <a class="btn btn-danger" id="del-btn" href="javascript:void(0)"><i
                                        class="fa fa-times"></i> 批量删除</a>
...
<!--新建/编辑规则-->
<div id="auth_rule-form" style="display: none;">
    <div style="width:380px;margin:15px auto;">
        <form class="form-horizontal" id="form1">
            <div class="form-group">
                <label for="title" class="col-sm-4 control-label">规则名称：</label>
                <div class="col-sm-8">
                    <input type="text" class="form-control" name="title" id="title" placeholder="">
                </div>
            </div>
            <div class="form-group">
                <label for="name" class="col-sm-4 control-label">规则标识：</label>
                <div class="col-sm-8">
                    <input type="text" class="form-control" name="name" id="name" placeholder="">
                </div>
            </div>
            <div class="form-group">
                <label for="condition" class="col-sm-4 control-label">附加规则：</label>
                <div class="col-sm-8">
                    <input type="text" class="form-control" name="condition" id="condition" placeholder="">
                </div>
            </div>
            <div class="form-group">
