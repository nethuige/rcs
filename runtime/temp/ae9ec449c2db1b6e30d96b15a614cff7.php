<?php if (!defined('THINK_PATH')) exit(); /*a:1:{s:60:"G:\EPP4\RCS\public/../application/admin\view\menu\index.html";i:1489728780;}*/ ?>
<!DOCTYPE html>
<html>
<head>
    <meta charset="utf-8">
    ...
    $(function () {
        ...
        var showSwitch = Common.InitSwitch("#show"); //是否显示switch
        initTable();
        //删除
        $("body").on("click",".del-btn",function(){
            ...
            var index = layer.confirm('确定要删除吗？', {
                  ...
                  btn: ['确定','取消']
                    ...
                    var params = {id:id};
                    $.post("<?php echo url('menu/delete'); ?>",params,function(data){
                        layer.close(index);
                        if(data.status){
        ...
        $("body").on("click",".create-btn",function(){
            ...
            Common.SetSwitch(showSwitch,true); //默认显示
            var that = $(this);
            var layerIndex = layer.open({
              ...
              yes: function(index, layero){
                    if($("#form1 #title").val()==""){
                        toastr.warning("请填写菜单名称","系统提示");
                        return false;
                    }
                    var params = $("#form1").serializeObject();
                    var pid  = that.data("pid");
                    params.pid = pid;
                    params.show = params.show==""?0:1;
                    params.status = params.status==""?0:1;
                    $.post("<?php echo url('menu/create'); ?>",params,function(data){
                        if(data.status){
        ...
        $("body").on("click",".edit-btn",function(){
            var id = $(this).data("id");
            var params = {id:id}
            $.get("<?php echo url('menu/edit'); ?>",params,function(data){
                $("#form1").setForm(data); //扩展方法,一键设置表单值
                //switch插件另外设置
                data.status=='启用' ? Common.SetSwitch(statusSwitch,true) :  Common.SetSwitch(statusSwitch,false);
                data.show=='显示'? Common.SetSwitch(showSwitch,true) :  Common.SetSwitch(showSwitch,false);
            }); 
            var layerIndex = layer.open({
              type: 1,
              title:'编辑菜单',
              ...
              yes: function(index, layero){
                    ...
                    params.show = params.show==""?0:1;
                    params.status = params.status==""?0:1;
                    $.post("<?php echo url('menu/edit'); ?>",params,function(data){
                        if(data.status){
                            layer.close(layerIndex);
                            toastr.success(data.msg,"系统提示");
                            $table.bootstrapTable('refresh', { silent: true });
    ...
    function initTable(setParams) {
        ...
        $table.bootstrapTable({
            ...
            columns: [{
                ...
                align: 'center',
                formatter:function(value,row,index){
                    return value=='隐藏'?'<span class="label">隐藏</span>':'<span class="label label-success">显示</span>'
                }
            }, {
                field: 'status',
                title: '状态',
        ...
        <div class="col-sm-12">
            <div class="ibox float-e-margins">
                <div class="ibox-title">
                    <h5>菜单管理 <small class="m-l-sm">菜单列表</small></h5>
                ...
                <div class="ibox-content">
                    ...
                    <div class="row">
                        <div class="col-sm-12">
                            <div id="toolbar">
                                <a class="btn btn-primary create-btn" href="javascript:void(0);" data-pid="0"><i class="fa fa-plus"></i> 创建菜单</a>
    ...
    <div style="width:380px;margin:15px auto;">
        <form class="form-horizontal" id="form1">
            <div class="form-group">
                <label for="title" class="col-sm-4 control-label">菜单名称：</label>
                <div class="col-sm-8">
            ...
            <div class="form-group">
                ...
                <div class="col-sm-8">
                  <input type="text" class="form-control" name="url" id="url" placeholder="如:admin/user/index">
                </div>
            </div>
            <div class="form-group">
                <label for="icon" class="col-sm-4 control-label">菜单图标：</label>
                <div class="col-sm-8">
                  <input type="text" class="form-control" name="icon" id="icon" placeholder="fa fa-xxx">
                </div>
            </div>
            <div class="form-group">
            ...
            <div class="form-group">
                <label for="show" class="col-sm-4 control-label">是否显示：</label>
                <div class="col-sm-8">
                  <input type="checkbox" id="show" name="show" class="js-switch" />
                </div>
            </div> 
            <div class="form-group">
