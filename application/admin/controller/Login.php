...
class Login extends Controller
{
    ...
    public function index(Request $request)
    {
        if ($request->isGet()) {
            ...
            $user = UserModel::where($where)->field("password",true)->find();
            if (empty($user)) {
                ...
                //验证通过,开始验证权限
                $auth = new Auth();
                $checkResult = $auth->check("system_login_check", $user['id']);
                if ($checkResult) {
                    $user['groups'] = $auth->getGroups($user['id']);
                    Session::set("userinfo", $user); //保存用户对象到session
                    Session::set("is_login", true); //保存用户对象到session
